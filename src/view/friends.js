...
const imgs = [
  ...
  require("../imgs/caocao.jpg"),
  require("../imgs/daojian.jpg"),
  require("../imgs/pushu.jpg")
];
  ...
  return (
    ...
    >
      <View>
        <Image
          source={item.avatar}
          style={{ width: 0.1 * width, height: 0.1 * width }}
        />
      </View>
      <View style={{ width: 0.8 * width, paddingLeft: 0.05 * width }}>
        <View>
          <Text style={{ fontSize: 20, fontWeight: "bold", color: "#4F88F2" }}>
            {item.name}
          </Text>
        </View>
        {arrs.map(item => (
          <View>
            <Text style={{ fontSize: 16, color: "black", lineHeight: 19 }}>
              {item}
            </Text>
          </View>
        ))}
        ...
        >
          {(item.imgs || imgs).map(
            (item, index1) =>
              (index1 + 1) % 3 ? (
                <TouchableWithoutFeedback
                  onPress={() => setActive(index1, index)}
                >
                  <View
                    style={{
                      width: 0.24 * width,
                      height: 0.2 * width,
                      marginRight: 0.014 * width,
                      marginBottom: 0.015 * width
                    }}
                  >
                    <Image
                      source={item}
                      style={{ width: 0.24 * width, height: 0.2 * width }}
                    />
                  </View>
                </TouchableWithoutFeedback>
              ) : (
        ...
        >
          ...
          >
            <TouchableWithoutFeedback onPress={() => likes()}>
              <View>
                <Text style={{ color: "pink" }}>❤️赞</Text>
              </View>
            </TouchableWithoutFeedback>

            <TouchableWithoutFeedback onPress={() => showComment(index)}>
              <View>
                <Text style={{ color: "pink" }}>🐂评论</Text>
              </View>
            </TouchableWithoutFeedback>
            {visible || widths ? (
              <View
...
class ImgMask extends Component {
  constructor(props) {
    ...
    this.animatedOpacity = new Animated.Value(0.8);
    ...
    this.animatedXY.addListener(value => {
      ...
      if (this.horizonMove) {
        ...
        if (this.onMoving) {
          this.animatedY.setValue(
            value.y < 0 ? 0 : -value.y / height * 0.25 * height
          );
          this.animatedOpacity.setValue(
            value.y < 0 ? 0.8 : (1 - value.y / (1.5 * height)) * 0.8
          );
          this.animatedScale.setValue(
            value.y < 0 ? 1 : 1 - value.y / (1.3 * height)
          );
    ...
    this.panResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        if (
          !this.state.horizonMove &&
          !this.onMoving &&
          Math.abs(gestureState.vy) < 0.0000000000006
        ) {
        ...
        Animated.event([
          null,
          { dx: this.animatedXY.x, dy: this.animatedXY.y }
        ])(evt, gestureState);
      },
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (this.state.horizonMove) {
          if (
            Math.abs(this._value.x) < 0.4 * width ||
            (active == 5 && this._value.x < 0) ||
            (active == 0 && this._value.x > 0)
          )
            ...
            Animated.timing(this.animatedValue, {
              toValue:
                this._value.x < 0
                  ? -(this.props.active + 1) * width
                  : -(this.props.active - 1) * width,
              duration: 300,
              easing: Easing.linear,
              useNativeDriver: true
            }).start(() => {
              this.initAnimatedVal =
                this._value.x < 0
                  ? -(this.props.active + 1) * width
                  : -(this.props.active - 1) * width;
              this.setState({ horizonMove: false });
          ...
          if (this._value.y < 0 || (this._value.y > 0 && this._value.y < 80)) {
            ...
            const x = 0.2 * width + (active % 3) * 0.255 * width + 0.12 * width;

            const needAddHeight = presentIndex
              ? boxHeightArr
                  .filter((item, index) => index <= presentIndex - 1)
                  .reduce((prev, next) => prev + next)
              : 0;
            const y =
              needAddHeight +
              imgsTopArr[presentIndex].y +
              ((active / 3) | 0) * 0.18 * width +
              0.1 * width -
              scrollTop;
            // const y = this.props.layouts.y + (active/3 | 0)*.18*width + .1*width;

            const imgx = 0.5 * width;
            const imgy = 0.38 * height;
            this.onMoving = false;
        ...
        >
          {imgs.map(
            (item, index) =>
              index == active ? (
                <Animated.View
                  style={[styles.imgBox, { left: index * width }, transformImg]}
                >
                  <Animated.Image
                    source={item}
                    style={{
                ...
                <Animated.View style={[styles.imgBox, { left: index * width }]}>
                  <Image
                    source={item}
                    style={{ width: 0.95 * width, height: 0.4 * height }}
                  />
                </Animated.View>
              )
          )}
            ...
            {imgs.map((item, index) => (
              <View
                style={{
                  ...
                  borderRadius: 5,
                  marginRight: 5,
                  backgroundColor: active == index ? "yellowgreen" : "#ffffff"
                }}
              />
            ))}
          </View>
...
class Friends extends Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      infos: [
        ...
        {
          name: "sumail Lei",
          content: "我今天去理发店理发花了300p\n日 心疼....\n晚上又开始了敲代码",
          time: "10分钟前",
          avatar: require("../imgs/qq1.jpg")
        },
        {
          name: "sumail Lei",
          content: "我今天去理发店理发花了300p\n日 心疼....😊\n晚上又开始了敲代码\n晚上又开始了敲代码",
          time: "10分钟前",
          avatar: require("../imgs/qq3.jpg")
        }
  ...
  voteMsg = index => {
    ...
    const changeInfo = {
      ...infos[index],
      likes: !infos[index].likes ? 1 : infos[index].likes + 1
    };
  ...
  addComment = index => {
    if (!this.state.comment) {
      this.setState({ showComment: false, presentIndex: null });
      return;
    }
    const infos = this.state.infos.slice();
    const changeInfo = {
      ...infos[index],
      comments: !infos[index].comments
        ? [this.state.comment]
        : [...infos[index].comments, this.state.comment]
    };
    infos[index] = changeInfo;
      ...
      <View style={{ alignItems: "center", width, height, paddingBottom: 110 }}>
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <DecPost
              ref={this["postRef" + index]}
              scrollTop={scrollTop}
              showComment={presentIndex =>
                this.setState({ showComment: true, presentIndex })}
              likes={() => this.voteMsg(index)}
              index={index}
              item={item}
              onImgLayout={this.onImgLayout}
              onBoxLayout={this.onBoxLayout}
              setActive={(active, presentIndex) => {
                this.setState({ active, presentIndex });
        ...
        {showComment ? (
          ...
          >
            ...
            />
            <TouchableWithoutFeedback
              onPress={() => this.addComment(presentIndex)}
            >
              <View
                style={{
                  width: 0.1 * width,
